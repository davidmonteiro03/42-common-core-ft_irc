...
void Server::getInfo(void) const
{
	...
	std::cout << std::endl
			  ...
			  << std::endl
			  << "\t" << std::setw(8) << std::left << "hostname" << ": " << hostname << std::endl
			  << "\t" << std::setw(8) << std::left << "ip" << ": " << ip << std::endl
			  << "\t" << std::setw(8) << std::left << "port" << ": " << _port << std::endl
			  << "\t" << std::setw(8) << std::left << "password" << ": '" << _password << "'" << std::endl
			  << std::endl;
}
...
bool Server::executePass(int fd, t_input input, t_string command)
{
	...
	_clients[fd]->login();
	if (oldClient.getStatus() != _clients[fd]->getStatus() && _clients[fd]->getStatus() == 1)
...
bool Server::executeNick(int fd, t_input input, t_string command)
{
	...
	if (oldClient.getStatus() != _clients[fd]->getStatus() && _clients[fd]->getStatus() == true)
		return welcomeMessage(_clients[fd]);
	return true;
}
...
{
	for (int fd = 0; fd <= _fdmax; fd++)
		if (FD_ISSET(fd, &_fds.all) && _clients[fd] != NULL && _clients[fd]->getStatus() == true && _clients[fd]->getNick() == name)
			return _clients[fd];
	return NULL;
}
		...
		if ((*it)->getName() == channel)
		{
			...
			for (size_t i = 0; i < flags.size(); i++)
			{
				if (std::strchr(MODE_CHARSET, flags[i][1]) == NULL)
					continue;
				if (std::strchr("it", flags[i][1]) == NULL && flags[i] != "-l")
				{
					if (j < arguments.size())
					{
...
void Server::removeEmptyChannels(void)
{
	...
	{
		if ((*it)->getUsers().size() == 0 && (*it)->getOperators().size() <= 1)
		{
			delete *it;
...
const char *Server::ErrorUsage::what() const throw() { return "Server: usage: ./ircserv <port> <password>"; }

const char *Server::InvalidPort::what() const throw() { return "Server: invalid port"; }

const char *Server::ErrorCreatingSocket::what() const throw() { return "Server: error creating socket"; }

const char *Server::ErrorBindingSocket::what() const throw() { return "Server: error binding socket"; }

const char *Server::ErrorListenSocket::what() const throw() { return "Server: error listening socket"; }

const char *Server::ErrorGettingHostname::what() const throw() { return "Server: error getting hostname"; }

const char *Server::ErrorGettingIP::what() const throw() { return "Server: error getting IP"; }
